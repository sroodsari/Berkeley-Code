...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        if (piece == null) {
            return false;
        } else if (row + piece.height() > height()) {
            return false;
        } else if (col + piece.width() > width()) {
            return false;
        }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int clearRowPts = nrows * _width;
        int clearColPts = ncols * _height;
        int clearedCells = clearRowPts + clearRowPts - (nrows * ncols);
        int bonus = (clearRowPts + clearColPts) * _streakLength;
        int totalPts = bonus + clearedCells;
        if (nrows != 0 || ncols != 0) {
            _streakLength += 1;
        } else {
            _streakLength = 0;
    ...
    boolean get(int row, int col) {
        if (_cells[row][col]) {
            return true;
        } else if (row  > _height || col > _width) {
            return true;
        }
        return false;
    }
