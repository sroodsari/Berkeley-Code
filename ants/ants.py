...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place, temp = self.place, 0
        nextPlace = place.entrance
        if place.bees:
            return choose_bee(place.bees)
        while nextPlace.entrance != Place.is_hive:
            if nextPlace.bees:
                if self.max_range and temp <= self.max_range:
                    return choose_bee(nextPlace.bees)
                elif self.min_range and temp >= self.min_range:
                    return choose_bee(nextPlace.bees)
            temp += 1
            nextPlace = nextPlace.entrance
        return choose_bee(nextPlace.bees)
        #while place != Place.is_hive:
        #    if place.bees and temp>=self.min_range and temp <= self.max_range:
        #        return choose_bee(place.bees)
        #    else:
        #        temp += 1
        #        place = place.entrance

        #while not bees and place != Place.is_hive:
        #    if self.min_range <= temp <= self.max_range:
        #        bees = place.bees
        #    place = place.entrance
        #    temp += 1
        #return choose_bee(bees)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        beeList = list(self.place.bees)
        if self.health <= 0:
            for i in beeList:
                i.reduce_health(bee,self.damage)
            self.place.remove_insect(self)
        # END Problem 5
...
class TankAnt(BodyguardAnt):
    ...
    implemented = True
    def __init__(self, health = 2):
        BodyguardAnt.__init__(self,health)
    def action(self, gamestate):
...
# END Problem EC
    ...
    def __init__(self, health=1):
        ...
        self.is_queen = QueenAnt.instances = 0
            ...
            while place.exit:
                ...
                if place.ant:
                    ...
                    if place.ant.is_container and place.ant.ant_contained:
                        damage(place.ant.ant_contained)
        # END Problem EC
