...
def choose(paragraphs, select, k):
    ...
    for i in range(len(paragraphs)):
        if select(paragraphs[i]):
            selectable.append(paragraphs[i])
    if k >= len(selectable):
        return ''
    else:
        return selectable[k]
...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words == []:
        ...
        return 0.0
    elif typed_words == reference_words:
        return 100.0
    else:
        for i in range(len(typed_words)):
            if i<len(reference_words):
                if typed_words[i] == reference_words[i]:
                    accuracyCount +=1
        return accuracyCount/(len(typed_words))*100
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for index in range(len(valid_words)):
            difference = diff_function(typed_word, valid_words[index], limit)
            if difference<min:
                min = difference
                correct = valid_words[index]
            index +=1
        if min>limit:
            return typed_word
        else:
...
def feline_flips(start, goal, limit):
    ...
    if start == goal:
        ...
        start = start[1:]
        goal = goal[1:]
        return 1 + feline_flips(start,goal, limit-1)
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        var = [1]*(len(times_per_player[i])-1)
        for j in range(1, len(times_per_player[i])):
            var[j-1]= times_per_player[i][j] - times_per_player[i][j-1]
        timed.append(var)
    return game(words, timed)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    lst = []*len(get_times(game))
    for i in player_indices:
        playerz = []
        lst.append(playerz)
    if not word_indices:
        return lst
    min, win = time(game,0,0), 0
    for j in word_indices:
        min = time(game, 0, j)
        for i in player_indices:
            if time(game, i, j) <= min:
                min = time(game, i, j)
                win = i
        fastest = word_at(game, j)
