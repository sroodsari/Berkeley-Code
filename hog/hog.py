...
def more_boar(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    playScoreLen = len(str(player_score))
    oppScoreLen = len(str(opponent_score))
    playMin = player_score%10
    playMax = player_score%10
    oppMin = opponent_score%10
    oppMax = opponent_score%10
    while playScoreLen >1:
        player_score = player_score//10
        if player_score%10<playMin:
            playMin = player_score%10
        elif player_score%10>playMax:
            playMax = player_score%10
        playScoreLen -=1
    while oppScoreLen >1:
        opponent_score = opponent_score//10
        if opponent_score%10<oppMin:
            oppMin = opponent_score%10
        elif opponent_score%10>oppMax:
            oppMax = opponent_score%10
        oppScoreLen -=1
    if playMax > oppMax and playMin < oppMin:
        return True
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who ==0:
            ...
            while more_boar(score0, score1) == True:
                num_rolls = strategy0(score0, score1)
                if num_rolls == 0:
                    score0 = score0 + piggy_points(score1)
                else:
                    score0 = score0 + roll_dice(num_rolls, dice)
                    say = say(score0, score1)
                if score0 >= goal:
                    return score0, score1
        else:
            ...
            if score1>= goal:
                return score0, score1
            while more_boar(score1, score0) == True:
                num_rolls = strategy1(score1, score0)
                if num_rolls ==0:
                    score1= score1 + piggy_points(score0)
                else:
                    score1 = score1 + roll_dice(num_rolls, dice)
                    say = say(score0, score1)
                if score1>= goal:
                    return score0, score1
        who = next_player(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            currentScore = score1
        temp = running_high
        difference = currentScore - last_score
        if difference > temp:
            print("Player", who, "has reached a new maximum point gain.", difference, "point(s)!")
        temp = difference
        return announce_highest(who, currentScore, max(difference, running_high))
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    numRoll = 10
    while count<=10:
        averageRoll = make_averaged(roll_dice, trials_count)
        thisMax = averageRoll(count, dice)
        if thisMax>maxAvg:
            maxAvg = thisMax
            maxRoll = count
        count+=1
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score = piggy_points(opponent_score) + score
    if more_boar(score, opponent_score):
        if opponenet_score > score:
            return 0
        else:
            return num_rolls
    else:
